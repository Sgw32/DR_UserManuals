...
\begin{longtblr}[
    ...
    \midrule
    22 (или 0) & \paramname{PARAMETER\_RPMCOEFFICIENT} & Калибровочный коэффициент оборотов. \\
    1 & \paramname{PARAMETER\_SPEEDCOEFFICIENT} & Калибровка скорости. \\
    2 & \paramname{PARAMETER\_COOLANTTHERMISTORB} & Бета-коэффициент датчика ОЖ. \\
    3 & \paramname{PARAMETER\_OILTHERMISTORB} & Бета-коэффициент датчика масла. \\
    4 & \paramname{PARAMETER\_AIRTHERMISTORB} & Бета-коэффициент датчика наружной температуры. \\
    5 & \paramname{PARAMETER\_TANKMINRESISTANCE} & Минимальное сопротивление датчика топлива (\si{\ohm}). \\
    6 & \paramname{PARAMETER\_TANKMAXRESISTANCE} & Максимальное сопротивление датчика топлива (\si{\ohm}). \\
    7 & \paramname{PARAMETER\_TAU\_COOLANT} & Постоянная фильтра температуры ОЖ. \\
    8 & \paramname{PARAMETER\_TAU\_OIL} & Постоянная фильтра температуры масла. \\
    9 & \paramname{PARAMETER\_TAU\_AIR} & Постоянная фильтра наружной температуры. \\
    10 & \paramname{PARAMETER\_TAU\_TANK} & Постоянная фильтра уровня топлива. \\
    11 & \paramname{PARAMETER\_MILEAGE} & Общий пробег. \\
    12 & \paramname{PARAMETER\_DAILY\_MILEAGE} & Суточный пробег. \\
    13 & \paramname{PARAMETER\_AUTO\_BRIGHTNESS} & Включение автоматической яркости. \\
    14 & \paramname{PARAMETER\_BRIGHTNESS\_LEVEL} & Ручной уровень яркости (0--15). \\
    15 & \paramname{PARAMETER\_TANK\_CAPACITY} & Ёмкость бака (литры). \\
    16 & \paramname{PARAMETER\_MFA\_STATE} & Активная страница MFA. \\
    17 & \paramname{PARAMETER\_BUZZER\_OFF} & Отключение зуммера (1 — выкл., 0 — вкл.). \\
    18 & \paramname{PARAMETER\_MAX\_RPM} & Диапазон тахометра (по умолчанию 8000). \\
    19 & \paramname{PARAMETER\_NORMAL\_RESISTANCE\_COOLANT} & Сопротивление датчика ОЖ при \SI{25}{\celsius}. \\
    20 & \paramname{PARAMETER\_NORMAL\_RESISTANCE\_OIL} & Сопротивление датчика масла при \SI{25}{\celsius}. \\
    21 & \paramname{PARAMETER\_NORMAL\_RESISTANCE\_AMB} & Сопротивление датчика наружного воздуха при \SI{25}{\celsius}. \\
    23 & \paramname{PARAMETER\_DOT\_OFF} & Поведение разделителя часов (0 — мигает, 1 — постоянно). \\
    24 & \paramname{PARAMETER\_BACKLIGHT\_ON} & Включение подсветки при ближнем свете. \\
    25 & \paramname{PARAMETER\_M\_D\_FILTER} & Константа медианного фильтра (наследие). \\
    26 & \paramname{PARAMETER\_COOLANT\_MAX\_R} & Порог полной шкалы температуры ОЖ. \\
    27 & \paramname{PARAMETER\_COOLANT\_MIN\_R} & Порог «1~bar» температуры ОЖ. \\
    31--33 & \paramname{PARAMETER\_MAINCOLOR\_[RGB]} & Компоненты цвета интерфейса (только \ReplicaNextShort{}). \\
    37 & \paramname{PARAMETER\_RPM\_FILTER} & Агрессивность фильтра оборотов. \\
    128 & \paramname{PARAMETER\_READ\_ADDITION} & Добавьте для чтения параметра. \\
    ...
    252 & \paramname{PARAMETER\_RESET\_DIGITAL} & Заводской сброс и инициализация памяти. \\
    \bottomrule
\end{longtblr}}
